        ...
        for episode, url in seasons2[season].iteritems():
            ...
            for tag in tags:
                if '.mp4' in str(tag.string).lower():
                    episodes.append(
                        [folder, season, episode, url, tag.get('href'), tag.string])
            ...
            with open(path, 'wb') as f:
                ...
                for chunk in progress.bar(r.iter_content(chunk_size=1024), expected_size=(total_length / 1024) + 1):
                    if chunk:
                        f.write(chunk)
                        f.flush()
            statement3 = "INSERT INTO '%s' (Season, Episode, Filename, Filesize, Download_time) VALUES(?, ?, ?, ?, ?)" % (
                folder)
            cur.execute(statement3, (item[1], item[2], item[5], total_length, time.strftime(
                "%d/%m/%Y|%H:%M:%S")))
        conn.commit()
